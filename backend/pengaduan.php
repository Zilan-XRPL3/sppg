...
<head>
    ...
    <title>Kotak Masuk - SPPG Center</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .status-badge { 
            ...
            font-size: 11px; font-weight: 800; text-transform: uppercase; margin-bottom: 20px; 
        }
        .status-pending { background: #FFF9E6; color: #F59E0B; }
        .status-proses { background: #E6F6FF; color: #0EA5E9; }
        ...
        .info-row i { color: var(--primary); width: 16px; text-align: center; }

        .card p { color: #64748B; font-size: 14px; line-height: 1.7; margin-bottom: 25px; }
        
        .btn-action { 
            margin-top: auto; padding: 15px; background: #F8FAFC; color: var(--dark); 
            text-decoration: none; border-radius: 16px; text-align: center; 
            font-weight: 700; border: 1px solid #E2E8F0; transition: 0.3s; 
        }
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="pengaduan.php" class="active"><i class="fas fa-envelope-open-text"></i> Kotak Masuk</a>
            <a href="pengaduan_selesai.php"><i class="fas fa-check-double"></i> Selesai</a>
        </div>
    ...
    <div class="main">
        <div class="header">
            <div>
                <h1>Kotak Masuk 📥</h1>
                <p style="color: var(--text-muted); margin-top: 5px;">Kelola pengaduan warga sekolah hari ini.</p>
            </div>
            <div style="background: white; padding: 12px 20px; border-radius: 18px; font-weight: 800; border: 1px solid #E2E8F0; font-size: 14px; display: flex; align-items: center; color: var(--dark);">
                <i class="far fa-calendar-alt" style="color: var(--primary); margin-right: 10px; font-size: 16px;"></i> 
                <span id="live-date">Menghitung...</span>
            </div>
        </div>

        <div class="grid">
            <?php while($row = mysqli_fetch_assoc($query)): 
                $status_class = ($row['status'] == 'proses') ? 'status-proses' : 'status-pending';
            ?>
            <div class="card">
                <div class="status-badge <?= $status_class ?>"><?= $row['status'] ?></div>
                <span style="font-size: 11px; font-weight: 800; color: var(--primary); margin-bottom: 8px; display: block;">#<?= $row['no_tiket'] ?></span>
                ...
                <a href="detail_pengaduan.php?id=<?= $row['id'] ?>" class="btn-action">Tindak Lanjut</a>
            </div>
            <?php endwhile; ?>
        </div>
    </div>
    ...
    <script>
        function updateLiveDate() {
            ...
            // Mengambil tanggal lokal dari komputer user (Hari ini pasti 05)
        ...
        // Jalankan langsung
        updateLiveDate();
        // Update setiap detik (opsional, tapi bagus untuk memastikan pergantian hari)
